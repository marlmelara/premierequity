      ...
      <div className="mx-auto max-w-7xl px-6 py-4 flex items-center justify-between gap-6">
        ...
        <nav className="hidden lg:flex items-center gap-8 text-sm font-medium text-neutral-700">
          {siteConfig.navLinks.map((link, i) => {
            const className =
              i === 0 ? "text-olive" : "hover:text-forest transition-colors";
            // Home routes home (client-side from subpages) and smooth-scrolls to
            // top when already home; section links are native anchors.
            return link.href === "/" ? (
              <Link
                key={link.href}
                href="/"
